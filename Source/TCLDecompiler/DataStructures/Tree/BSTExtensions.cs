...
namespace TCLDecompiler {
	internal static class BSTExtensions {
		...
		internal static (BSTNodeBase<T> node, int position) Find<T>(this BSTNodeBase<T> current, T value) where T : IComparable {
			int position = 0;
			...
			while (true) {
				...
				if (compareResult > 0) {
					...
					position += (current.Left?.Count ?? 0) + 1;
